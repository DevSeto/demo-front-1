...
@Component({
    ...
    template: `
        <div class="form-inline ng2-datetime">
            ...
            <div [ngClass]="{ 'form-group': true,  'bootstrap-timepicker timepicker': true }">
...
export class NKDatetime implements ControlValueAccessor, AfterViewInit, OnDestroy, OnChanges {
    ...
    @Input('isSelected') isSelected: number;
    ...
    showTimepicker() {
        ...
        if (document.getElementsByClassName('bootstrap-timepicker-widget').length && this.isSelected){
            document.getElementsByClassName('bootstrap-timepicker-widget')[0].classList.add('selectedPopUp');
        }else{
            document.getElementsByClassName('bootstrap-timepicker-widget')[0].classList.remove('selectedPopUp');
        }
    }
    ...
    private init(): void {
        if (!this.datepicker && this.datepickerOptions !== false) {
            const options = jQuery.extend({enableOnReadonly: !this.readonly}, {container: '#hours-popup'});
        ...
        if (!this.timepicker && this.timepickerOptions !== false) {
            ...
            this.timepicker
                .on('changeTime.timepicker', (e: ITimepickerEvent) => {
                    ...
                    if (!isDate(this.date)) {
                        this.date = new Date();
                        this.updateDatepicker(this.date, hours);
                    }
    ...
    private updateDatepicker(date?: any, hours ?: any) {
